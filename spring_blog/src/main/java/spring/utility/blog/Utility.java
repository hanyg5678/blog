...
public class Utility{
	private static final String root ="/blog";
	  ...
	  public static int rcount(int num, IDAO idao){
          return idao.rcount(num);
	  }
...
//	    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	...
	/** 
	  ...
	  * @return 페이징 생성 문자열
	  */ 
	 public static String paging(int totalRecord, int nPage, int recordPerPage, String url, int bbsno, int nowPage, String col, String word, String no){ 
	   int pagePerBlock = 10; // 블럭당 페이지 수 
	   int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
	   int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
